...
> **Note:** The simplest form of orchestration is turning an agent into a tool using `AgentTool`. When the root LLM identifies an intent matching the sub-agent's description, it calls the `AgentTool`, transferring execution context to the sub-agent.
...
```mermaid
flowchart LR
    Input([User Input]) --> Agent1[Agent 1: Research]
    Agent1 --> Agent2[Agent 2: Analyze]
    Agent2 --> Agent3[Agent 3: Write]
...
```mermaid
flowchart TD
    Input([User Input]) --> Split{Broadcast}
    Split --> Agent1[Agent 1: Optimist]
    Split --> Agent2[Agent 2: Pessimist]
    Split --> Agent3[Agent 3: Pragmatist]
    Agent1 --> Aggregate{Aggregate}
    Agent2 --> Aggregate
    Agent3 --> Aggregate
    Aggregate --> Output([Combined Output])
```
...
```mermaid
flowchart TD
    Input([User Input]) --> Drafter[Agent A: Drafter]
    Drafter --> Critic[Agent B: Critic]
    Critic -- Score < 8 --> Drafter
    Critic -- Score >= 8 --> Escalate[Escalate Tool]
    Escalate --> Output([Final Output])
```
